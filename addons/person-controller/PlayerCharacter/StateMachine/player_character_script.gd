...
func check_interactable():
	...
	var to = from + forward * interact_distance
	...
	if not obj.get_meta("interactable", true) and not obj.is_in_group("interact"):
		return
	# точка попадания
	var hit_pos = result.position

	# нормализованный вектор от камеры к точке
	var dir_to_hit = (hit_pos - from).normalized()

	# проверяем угол: чем ближе dot к 1.0 — тем точнее центр
	var dot = forward.dot(dir_to_hit)

	# предел точности: 0.995 ≈ ~5 градусов, можно уменьшить
	if dot < 0.999: # это очень жёстко: ~2 градуса
		return
		
	if Input.is_action_just_pressed("interact"):
		var p = obj.get_parent()
		if p.has_method("_interact"):
			print("_interact")
			p._interact()
